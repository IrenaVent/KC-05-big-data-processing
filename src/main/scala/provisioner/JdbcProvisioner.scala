...
object JdbcProvisioner {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      statement.execute("CREATE TABLE IF NOT EXISTS bytes (date TIMESTAMP, id TEXT, value BIGINT, type TEXT)")

      println("Creando la tabla bytes_hourly (date TIMESTAMP, id TEXT, value BIGINT, type TEXT)")
      statement.execute("CREATE TABLE IF NOT EXISTS bytes_hourly (date TIMESTAMP, id TEXT, value BIGINT, type TEXT)")

      println("Creando la tabla user_quota_limit (email TEXT, usage BIGINT, quota BIGINT, date TIMESTAMP)")
      statement.execute("CREATE TABLE IF NOT EXISTS user_quota_limit (email TEXT, usage BIGINT, quota BIGINT, date TIMESTAMP)")
